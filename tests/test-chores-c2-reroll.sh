...
printf 'different\n' > "$OLD"; touch -d '45 days ago' "$OLD"; touch "$TEST_TMP/.remember/remember.md"
MAUDE_REROLL=on bash "$CH" run c2-shelves >/dev/null 2>&1
...
touch -d '90 days ago' "$TEST_TMP/.remember/now.md" "$TEST_TMP/.remember/archive.md"
MAUDE_REROLL=on bash "$CH" run c2-shelves >/dev/null 2>&1

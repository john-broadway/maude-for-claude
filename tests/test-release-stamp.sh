...
BQ_RE='> [*][*]Version:[*][*] [0-9][0-9A-Za-z.-]*'   # bracketed stars: no backslash for any awk to eat
...
fixture "$F"
stamp_header "$F" "$VER_RE" "<!-- Version: 0.99.0 -->"
assert_eq "$(wc -l < "$F" | tr -d ' ')" "8" "no lines added or lost"
...
I="$TEST_TMP/inode.md"
printf '<!-- Version: 0.1.0 -->\nbody\n' > "$I"
INO_BEFORE="$(file_inode "$I")"
stamp_header "$I" "$VER_RE" "<!-- Version: 0.9.9 -->"
INO_AFTER="$(file_inode "$I")"
...
assert_eq "$(find "$TEST_TMP" -maxdepth 1 -name '.stamp.*' | wc -l | tr -d ' ')" "0" "temp cleaned up"

test_start "stamping preserves the file's mode"
M="$TEST_TMP/mode.md"
printf '<!-- Version: 0.1.0 -->\nx\n' > "$M"; chmod 644 "$M"
stamp_header "$M" "$VER_RE" "<!-- Version: 0.9.9 -->"
assert_eq "$(file_mode "$M")" "644" "mode unchanged by the stamp"
...
assert_eq "$(find "$CH" -maxdepth 1 -name 'link*.md' -type f | wc -l | tr -d ' ')" "0" "every link is still a link"
...
assert_eq "$(find "$TEST_TMP" -maxdepth 1 -name '.stamp.*' | wc -l | tr -d ' ')" "0" "temp cleaned up on the failure path"

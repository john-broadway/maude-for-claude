...
test_start "a TMPDIR with a trailing slash (macOS exports one) gives the file a root with no double slash"
SLASHED="$TEST_TMP/slashed"; mkdir -p "$SLASHED"
printf '#!/usr/bin/env bash\n. "%s/lib.sh"\nsetup_test_env\nprintf "ROOT=%%s TMP=%%s\\n" "$TEST_TMPROOT" "$TEST_TMP"\nprintf "1 passed, 0 failed\\n"\nexit 0\n' "$TESTS_DIR" > "$RUNDIR/test-fixture.sh"
OUT="$(TMPDIR="$SLASHED/" bash "$RUNDIR/test-fixture.sh" 2>&1)"
assert_contains "$OUT" "ROOT=$SLASHED/maude-tests." "the root sits under the given TMPDIR"
assert_not_contains "$OUT" "//" "and no path carries a double slash"

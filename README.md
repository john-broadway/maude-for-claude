...
- **`/maude:found`** writes the house-map · **`/maude:wake` / `/maude:rest`** orient on arrival / close the loop with a save fan-out · **`/maude:verify`** runs the readiness audit (version sync, JSON, links, dates, **references to cut commands, an un-condensed changelog** — leads with a count, never a verdict) · **`/maude:cushions`** flips the cushions — unpushed commits, uncommitted files, sole-copy repos, aging scratch — reports value candidates, never deletes · **`/maude:lint`** walks the memory vault the way the flip walks the cushions — broken index links, unwritten pointers, stale open-flags, superseded notes the index still serves — mechanical checks by script, judgment by the reader, report-first (archives/letters/dailies never touched) · **`/maude:conscience`** is the gate's deliberate release valve · **`/maude:teach`** tells her a fact about you · **`/maude:receipts`** prints the measured table — what she caught, counted honestly from her own records (stated window, friction separated from value, no percentages: there's no honest denominator for disasters that didn't happen). Plus `save`, `notice`, `check-on-claude`. Full surface in [`commands/`](commands/).

---

## What's new

**v0.24.0 (2026-07-22) — the bill, the switch, and the walk.** The field-issue backlog, closed. **The token ledger (#49):** her hooks inject context every turn and that bill was invisible — now every context-injecting hook class logs its spend to the trace (hook + bytes, metadata only), `/maude:receipts` grows a "what she cost" table beside her catches (tokens derived as bytes/4, marked approximate, never stored; savings stay event counts — a saved-token number has no honest denominator), and the pre-registered suspect is retired: the vault pager no longer fires on machine-generated turns (task notifications, command echoes) — a notification is not a question. **The kill switch (#45):** `MAUDE_PROBE=off` — the loopback liveness probe was the one autonomous feature the homeowner couldn't turn off; no sockets, no cache, documented beside its siblings. **The lint ritual (#42):** `/maude:lint` walks the memory vault the way the cushion-flip walks the repos — broken index links, unwritten pointers (a backlog, not errors), index size vs cap, stale open-flags, superseded notes the index still serves; mechanical checks by script, judgment by the reader, report-first, archives/letters/dailies never touched. First dogfood run on a real 490-file vault cut pointer noise 78→12 and caught the index over its cap. One new test file; fleet 47.

**v0.23.0 (2026-07-22) — whose session it was.** Run a fleet — several concurrent Claude sessions sharing one project root — and her aggregate surfaces, built for one session at a time, crossed the streams: the wake brief served one session another's "where you left off," the catch-digest pooled everyone's catches into one anonymous count, and drift-watch could whisper "Claude is stuck grepping" about a *neighbor's* Claude. Caught live, twice in one day. Now every trace entry carries a `session` label (the tmux session name — the name you actually think in — else a session-id prefix, else `solo`) and the commentary ties itself to it: drift-watch counts only the current session's events with per-session cooldowns, the digest labels each session's catches when two or more share the window (`pacioli: 1 block · proximo: 2 verify-flags`), and "Where you left off" declares its source (the entry's own label, or `workspace-wide`) instead of masquerading as this session's state. A lone stream keeps the plain format — solo installs read exactly as before, and nothing leaves the house.

**v0.22.0 (2026-07-17) — every door, and an honest count.** She reaches more houses and shows her work. **macOS is first-class now (#39):** the hooks carried GNU-isms (`stat -c`, `date -d`, GNU `touch -d`, a `sed` word-boundary) that BSD userland silently rejects — so on a Mac her redaction emitted *nothing*, the continuity guard stayed quiet, and the eye ran dark for want of a `timeout` binary. Each now speaks both dialects (GNU first, BSD fallback, tagged shims a lint holds in place), and a `macos-latest` CI leg proves it on real BSD — a leg that failed red on the first honest run and caught what the Linux-only fakes could not. **`/maude:receipts` — the measured table (#43):** the measure of a protector is the disaster that didn't happen, and that was a sentence; now it's a table — sole-copy saves, gate blocks, drift caught at the flip, counted from her own trace with friction fenced from value by rule, no percentages (a rate needs a denominator; disasters that never happened have none), payloads classified but never surfaced. Its own review caught the first headline counting routine success-stamps as catches — a roughly tenfold overcount (~5,100 where ~525 were real) — and the fix is why the table is small and true. Plus a `PRIVACY.md` that says it plainly (everything stays in your house), a one-button ship rail, and a social card so a link to her unfurls with her face. Three new test files; fleet 46.

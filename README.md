...
**v0.32.0 (2026-09-14) - the fleet was one counter, and the whisper died by the clock.** A second site wrote down what Maude costs them, and two of the findings were defects in the plugin, not in their setup. The run governor kept one counter per project, ticked by every session and every subagent under it and reset only by a human prompt, which no subagent ever receives; an overnight fleet pooled its calls into one ceiling and every worker was blocked together, the finished ones unable to write their reports. The harness stamps an agent id on every tool hook fired inside a subagent, probed live rather than read from the docs, so each agent is now governed on its own count and its own clock from dispatch, a human turn drops every slot, a stopped agent drops its own, and the pause a subagent sees tells it the one move it has: finish and report. The eye's whisper was dropped after five wall-clock minutes, but pickup only happens at your next prompt, so a ceiling shorter than your turn cadence dropped every whisper while nothing had moved: none of ten delivered here over three days, six of six at the other site. Age is now counted in tool calls since the blink that produced it, forty by default (`MAUDE_EYE_WHISPER_TTL_ACTIONS`), and the seconds ceiling is opt-in. Two lens rounds: the first found the fix itself unlocked, sixty parallel ticks landing four, now under the lock with the blink kept off it; the second found no blocker and two bounded residuals, both named in the code. One behavior change to know: the main thread's count no longer includes its workers' calls, so for a fleet the ninety-minute clock is the operative ceiling.

**v0.31.0 (2026-09-03) - the law arrives at the moment the work touches its class.** John named three bodies of design law and asked Maude to hold Claude to them: the thirty Laws of UX, Codd's rules with third normal form as the floor, and the laws of memory, human and machine. They lived on a website and in textbooks, and nothing consulted either at the moment a `CREATE TABLE`, a button, or a cache was written. Now three rulebooks ship as static data, joined by a family key so third normal form and provenance read as one law from two seats, and a rail called from hooks that already fire says the law once on the first write of a class, lints a schema on the spot for the shapes its own text can break (no key, a repeating group, a fact hung on the wrong key, an id with no REFERENCES), and asks once at the commit if no design named its laws. The stamp is strict on purpose: a heading that names the rulebook and a canonical name, in a prose document that is not a fixture, never a commit message, because a false stamp is a false all-clear. The memory rulebook ships as a draft, looked up not recalled, until John cuts it. The honest seam is in the code: for a schema she catches objective shapes; for UI and memory she catches absence and hands presence to the adversarial lens with the named list as its brief. Every task got its own review, then one whole-branch lens and one fix wave, and that last lens found a class older than the branch: `python3 -m` puts the project's own directory first on the import path, so the new call sites run in the interpreter's safe-path mode and the older ones are filed. Kill switch `MAUDE_RULES=off`; `/maude:rules` prints the checklist.

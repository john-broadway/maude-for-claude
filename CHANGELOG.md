...
the flock branch honours; each now has its BSD form or the library's own shim. The next
run hung six hours: the mkdir lock fallback (the path a box without flock takes) spun
forever when the lock dir could not be made at all, and 22 sole-copy pins went red because
macOS exports TMPDIR with a trailing slash, the suite built its roots as "$TMPDIR/x", and
a protected root carrying "//" matched nothing against the gate's slash-collapsed command
view. The fallback refuses when no lock can be made; a configured root is collapsed the
way the command is; the suite strips the slash. Two hook lines changed, both in the common
library. The install-smoke gate, which runs the shipped archive's own fleet, discarded
that fleet's output and reported a red with nothing to read; it prints the failing files
and the summary now.
